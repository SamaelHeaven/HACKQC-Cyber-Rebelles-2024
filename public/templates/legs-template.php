</main>
<footer class="bg-body-tertiary">
    <div class="container py-3">
        <div class="d-flex footer-content">
            <div class="d-flex flex-column">
                <div class="pt-2 footer-logos mx-auto">
                    <a href="index.php"><img src="/public/images/fitQuestLogo.png" alt="logo" class="footer-logo"></a>
                    <a href="index.php"><img src="/public/images/logo_equipe.webp" alt="logo" class="footer-logo"></a>
                </div>
                <div class="pt-3 mx-auto copyright-text">© Cyber-Rebelles, 2024</div>
            </div>

            <div class="pt-4 mx-auto d-flex info-footer">
                <div>
                    <h2 class="mb-4 fs-6 footer-text">NOUS JOINDRE</h2>
                    <p><em class="fa-solid fa-envelope me-2"></em>test@exemple.ca</p>
                </div>
                <div class="social-networks footer-text">
                    <h2 class="mb-3 fs-6">NOUS SUIVRE</h2>
                    <a href="https://github.com/SamaelHeaven/HackQC-Cyber-Rebelles-2024" target="_blank"><em class="fa-brands fa-github"></em></a>
                    <a href="https://devpost.com/software/hackqc-2024" target="_blank"><img src="/public/images/devposticon.ico" alt="devpost-icon" class="devpost-icon"></a>
                </div>
            </div>
        </div>
    </div>
</footer>
</body>
